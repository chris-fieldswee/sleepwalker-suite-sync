...
export function useReceptionActions(
  ...
  const handleReportNewIssue = async (roomId: string, description: string, photo: File | null): Promise<boolean> => {
        ...
        try {
            ...
            const taskToInsert = {
                ...
                guest_count: 1,
                status: 'repair_needed' as const, // Keep status consistent internally maybe?
  ...
  // *** UPDATED FUNCTION: handleUpdateIssue ***
  const handleUpdateIssue = async (
      taskId: string,
      updates: Partial<{ // Define updates type inline for clarity
          issue_flag: boolean | null; // Changed from status
      ...
      try {
           ...
           // Optionally, if marking as fixed (issue_flag: false/null), set status to 'done'?
           // if (updates.issue_flag === false || updates.issue_flag === null) {
           //    finalUpdates.status = 'done';
           //    finalUpdates.stop_time = new Date().toISOString(); // Mark completion time
           // } else {
           //    // If marking as 'to fix', ensure status is appropriate (e.g., 'repair_needed' or 'todo')
           //    finalUpdates.status = 'repair_needed'; // Or 'todo' if preferred
           // }
