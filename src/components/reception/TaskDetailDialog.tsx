...
import { CalendarDays, Clock, User, DoorOpen, BedDouble, StickyNote, AlertTriangle, Image as ImageIcon, Edit2, X, Timer, Minus, Plus } from "lucide-react"; // Added Timer, Minus, Plus
...
const getAvailableCleaningTypes = (roomGroup: RoomGroup | null): CleaningType[] => {
  if (!roomGroup) return ['W', 'P', 'T', 'O', 'G', 'S']; // Return all if group unknown initially
...
interface Task {
  ...
  // Removed unused time fields from interface for clarity in this context
...
export function TaskDetailDialog({
    ...
    const handleFieldChange = <K extends keyof EditableTaskState>(field: K, value: EditableTaskState[K]) => {
        ...
        if (field === 'roomId' && typeof value === 'string') {
            ...
            if (editableState && !newAvailableTypes.includes(editableState.cleaningType)) {
                setEditableState(p => p ? { ...p, cleaningType: newAvailableTypes[0] || 'W' } : null); // Default to 'W' if no types
            }
        }
    };
    ...
    const handleSave = async () => {
        ...
        if (editableState.notes && editableState.notes.length > 2000) {
         ...
         // Optional: Validate timeLimit if needed (e.g., must be positive)
        ...
        // Handle null comparison correctly for timeLimit
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 py-4 overflow-y-auto px-1 flex-grow">
                    ...
                    {/* Column 2: Simplified Time & Issue Info */}
                    <div className="space-y-4">
                        {/* Time Limit (Editable/View) */}
                        <div className="space-y-1">
                        ...
                        {/* Actual Time & Difference (View Only, typically calculated) */}
                        {(task.actual_time !== null || task.difference !== null) && (
                            <Card className="bg-muted/30">
                                <CardHeader className="p-3">
                                    <CardTitle className="text-base flex items-center gap-1"><Timer className="h-4 w-4"/>Measured Time</CardTitle>
                                </CardHeader>
                                <CardContent className="p-3 pt-0 grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                                    <div><span className="text-muted-foreground">Actual:</span> {task.actual_time ?? '-'} min</div>
                                    {task.difference !== null && (
                                        <div className={cn("font-medium", task.difference > 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400")}>
                ...
                <DialogFooter>
                    {/* ... (keep existing footer logic for Edit/Save/Cancel/Close) ... */}
                    {isEditMode ? (
                        <>
