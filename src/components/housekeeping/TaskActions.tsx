  ...
  return (
    <div className="flex gap-2 flex-wrap">
      ...
      {(task.status === "todo" || task.status === "repair_needed") && (
        <Button size="sm" onClick={() => onStart(task.id)} disabled={!!activeTaskId} className="bg-brand-primary hover:bg-brand-primary/90 text-white">
      ...
      {task.status === "paused" && (
        <>
          ...
          <Button size="sm" onClick={() => onResume(task.id)} disabled={!!activeTaskId} className="bg-brand-primary hover:bg-brand-primary/90 text-white">
